...
export const CountryCache = new Map();

export async function loadChannel(countryCode, randomSort = false) {
	let channels = [];

	if (CountryCache.has(countryCode)) {
		channels = CountryCache.get(countryCode);
		if (randomSort) {
			return shuffle([...channels]);
		}
	}

	const _playlistUri = AppConfig.getInstance().config.playlistUri;
	const _playlistFallbackUrl1 = AppConfig.getInstance().config.playlistFallbackUrl1;
	...
	CountryCache.set(countryCode, channels);
