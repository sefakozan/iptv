...
// Add missing forward decls to avoid implicit declarations
...
// Add: acquire item with its bucket to manage in_use safely
static int cache_acquire(const char *url, cache_item_t **out_item, cache_bucket_t **out_bucket) {
  int bucket_idx = cache_hash(url);
  cache_bucket_t *bucket = &g_cache_buckets[bucket_idx];
  pthread_mutex_lock(&bucket->mutex);
  for (int i = 0; i < bucket->count; i++) {
    if (strcmp(bucket->items[i].url, url) == 0) {
      ...
      bucket->items[i].in_use = 1;
      *out_item = &bucket->items[i];
      *out_bucket = bucket;
      pthread_mutex_unlock(&bucket->mutex);
      pthread_mutex_lock(&g_stats_mutex);
      g_stats.cache_hits++;
...
// Add: release callback for evbuffer_add_reference to clear in_use when send completes
typedef struct {
  cache_item_t *item;
  pthread_mutex_t *mtx;
} cache_ref_cookie_t;

static void cache_ref_release(const void *data, size_t datalen, void *extra) {
  cache_ref_cookie_t *ck = (cache_ref_cookie_t*)extra;
  if (ck && ck->item && ck->mtx) {
    pthread_mutex_lock(ck->mtx);
    ck->item->in_use = 0;
    pthread_mutex_unlock(ck->mtx);
  }
  free(ck);
}
...
static void cache_put(const char *url, uint8_t *data, size_t size) {
  ...
  if (bucket->count >= bucket->capacity) {
    ...
    if (oldest_idx >= 0) {
      ...
      s_strlcpy(bucket->items[oldest_idx].url, url, sizeof(bucket->items[oldestidx].url));
...
static void upstream_chunk_cb(struct evhttp_request *up, void *arg) {
  ...
  if (cx->is_m3u8) {
    ...
    // First copy to cache (before draining input)
    if (cx->do_cache) {
    ...
    // Then stream upstream buffer directly (drains input)
...
static void segment_handler(struct evhttp_request *req) {
  ...
  // Cache lookup with safe acquire + reference release
  cache_item_t *it = NULL;
  cache_bucket_t *bucket = NULL;
  if (cache_acquire(target, &it, &bucket)) {
    struct evbuffer *buf = evbuffer_new();
    if (buf) {
      cache_ref_cookie_t *ck = (cache_ref_cookie_t*)malloc(sizeof(cache_ref_cookie_t));
      if (ck) { ck->item = it; ck->mtx = &bucket->mutex; }
      evbuffer_add_reference(buf, it->data, it->size, cache_ref_release, ck);
...
static void set_cpu_affinity(int worker_id) {
  ...
  if (ncpu <= 0) ncpu = 1;
  // Spread by PID to avoid all workers pinning to CPU 0
  int cpu = (int)(getpid() % ncpu);
  CPU_SET(cpu, &cpuset);
